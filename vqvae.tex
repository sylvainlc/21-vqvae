...
The parameters $\varphi$ are estimated through gradient descent during the training.

\paragraph{Quantization} Consider a codebook of $K$ vectors $e_{1:K} \in \mathbb{R}^n$, we compute $q_\varphi(z_q^{1:m}|z_e^{1:m})$ through a recurrent neural network.
We initialize the hidden state to zero $h^0 = 0$, and compute following states recursively, $\forall 1 \leq j \leq m$
\begin{align*}
    h^j &= \tanh(W_z z_e^j + W_h h^j + b) \\
    p^j &= \texttt{softmax}(- \| e_k - h^j\|^2_{1:K})
\end{align*}
where $W_z, W_h$ and $b$ are weight matrices and vector of the neural network.

In the specific conditions where $W_z = I_n$ and $W_h = b = 0$, these equations lead to
$$
p^j = \texttt{softmax}(- \| e_k - \textcolor{red}{\tanh(z_e^j)} \|^2_{1:K})
$$
