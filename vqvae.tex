...
\paragraph{Encoder - mixture vq-vae.} Consider an encoding function $f_\theta$ (proper neural network-based encoder  to be defined when the model is fixed) and write
$$
...
\textcolor{red}{we keep the exact same $f_{\varphi}$ and therefore the same $(z_\rme^1,\ldots, z_\rme^m)$ than in the original vq-vae. This means that $\varphi$ is not estimated at first and the data representation is optimized during an other phase.}
...
$$
q_\phi (z_q^{k} = i |X^{1:n},z_\rme^{1:m}) = \mathrm{Softmax}\left(\rme^{-\|z_\rme^{k}-\rme_i\|^2_{\Gamma}}\right) \eqsp,
$$
where $\|z_\rme^{k}-\rme_i\|^2_{\Gamma} = (z_\rme^{k}-\rme_i)^T\Gamma (z_\rme^{k}-\rme_i)$. This mean-field variational distribution assumes that the latent $(z_q^1,\ldots, z_q^m)$ are independent given $(X^1,\ldots,X^n)$ and $(z_\rme^1,\ldots, z_\rme^m)$. Here, the unknown parameters to estimate are 
$$
\phi = \left\{\rme_1,\ldots,\rme_K,\Gamma\right\}\eqsp.
$$

\paragraph{Decoder - mixture vq-vae.} 
...
$$
p_{\theta}(z_\rme,X|z_q = i) = \varphi_{\rme^i,\Gamma}(z_\rme)p_{\theta}(X|z_\rme)\eqsp,
$$
where $\varphi_{\rme^i,\Gamma}$ is the Gaussian probability density function and $p_{\theta}(X|z_\rme)$ is a decoder inspired by standard vq-vae.
