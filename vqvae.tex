...
\paragraph{Quantization} Consider a codebook of $K$ vectors $\rme_{1:K} \in \mathbb{R}^d$, $d \ll D$, we compute $q_\varphi(z_q^{1:m}|z_e^{1:m})$ through a recurrent neural network.
...
\begin{align*}
    ...
    p^j_k &= \texttt{softmax}(- \| \rme_k - h^j\|^2), \quad \forall 1 \leq k \leq K
\end{align*}
...
\begin{align*}
q_\varphi(z_q^{1:m} | z_e^{1:m}) &= \prod_{j=1}^m p_\theta(z_q^j|z_e^{1:j}) \\
				&= \prod_{j=1}^m \texttt{Cat}(\{p^j_k\}_{k=1}^K)
\end{align*}
