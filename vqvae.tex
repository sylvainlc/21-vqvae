...
The denoising process amounts then to sampling from the bridge associated with the SDE, i.e. sampling $U_q^s$ given $U_q^0$ and $U_q^t$ where $0<s<t$. The law of this Bridge is explicit for the Brownian diffusion and the Ornstein-Uhlenbeck diffusion. Following the same steps as in the previous section, 
$$
...
$$
\tilde \mu_{t|t-1,0} = \left(U_q^0\mathrm{e}^{-\theta\delta(t-1)} +U_*(1-\mathrm{e}^{-\theta\delta(t-1)})\right)
$$
and variance
$$
\tilde \sigma^2_{t|t-1,0} = \left(\frac{1}{(1-\mathrm{e}^{-2\theta(t-1)\delta})/(2\theta)} + \frac{\mathrm{e}^{-2\theta\delta}}{(1-\mathrm{e}^{-2\theta\delta})/(2\theta)}\right)^{-1}\,.
$$
