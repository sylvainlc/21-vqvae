...
$$
where $q_{\varphi}(Z_q^0 | Z_e) = q_{\varphi}(Z_q^0 | X)$, having $Z_e = f_\eta(X)$ similarly as in  previous sections. The model $q_{\varphi}(Z_q^t | Z_q^{t-1}, Z_e)$ gradually adds Gumble noise vectors $\mathbf{G}^t = \log(-\log(\mathcal{U}(0,1)))$:
\begin{align*}
