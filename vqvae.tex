...
\begin{align*}
p_{\theta}(Z_q^{0:T},U_q^{0:T}, X) &= p_{\theta}(Z_q^T,U_q^T) \left\{\prod_{t=0}^{T-1}p_{\theta}(Z_q^t,U_q^t|Z_q^{t+1},U_q^{t+1})\right\}  p_{\theta}(X|Z^{0:T}_q,U_q^{0:T})\,,\\
&= p_{\theta}(U_q^T) p_\theta(Z_q^T|U_q^T)\left\{\prod_{t=0}^{T-1}p_{\theta}(U_q^t|U_q^{t+1})p_\theta(Z_q^t|U_q^t)\right\}p_{\theta}(X|Z^{0}_q)\,.
\end{align*}
In this setting, $\{U_q^t\}_{T\leqslant t\leqslant 0}$ is the continuous latent state solution to a Stochastic Differenctial Equation (SDE) and conditionally on $\{U_q^t\}_{T\leqslant t\leqslant 0}$ the $\{Z_q^t\}_{T\leqslant t\leqslant 0}$ are independent with discrete distribution with support $(\rme^1,\ldots,\rme^K)$.  The prior distribution of  $U_q^T$ is assumed to be uninformative and this is the denoising process which provides the final latent state $Z_q^0$ which is used in the decoder, i.e. the conditional law of $X$ given the latent states. 
Most approaches \cite{} using diffusion probabilistic models for $\{U_q^t\}_{T\leqslant t\leqslant 0}$ are based on a fixed diffusion process. A notable exception is \cite{} where the authors learned a Gaussian diffusion process. This specific choice was motivated by the design of the generative model which required to "invert" the diffusion process, i.e. to sample from a bridge associated with the SDE, which is explicit only for very few classes of diffusion.

As the conditional law $p_{\theta}(Z_q^{0:T},U_q^{0:T}| X)$ is not available explicitly, we consider a variational approach to approximate it. Consider first an encoding function $f_\eta$ and write $Z_\rme= f_\eta(X)$. Then, consider the following variational familly:
$$
q_{\varphi}(Z_q^{0:T},U_q^{0:T}| X) = q_{\varphi}(U_q^0 | Z_\rme)q_{\varphi}(Z_q^0|U_q^0) \prod_{t=1}^T\left\{ q_{\varphi}(U_q^t|U_q^{t-1})q_{\varphi}(Z_q^t|U_q^t)\right\}\,.
$$
Using the auxiliary variational inference framework (AVI) and following \cite{}, define the loss function:
$$
\mathcal{L}(\theta, \varphi) = \mathbb{E}_{q_\varphi} \left[ \log p_\theta(U_q^{0},Z_q^{0}|U_q^{0},Z_q^{0}) - \sum_{t=1}^{T}\mathrm{KL}(q_\varphi(Z_q^{t-1},U_q^{t-1}|Z_q^t,U_q^{t},Z_q^0,U_q^{0})\|p_\theta(Z_q^{t-1},U_q^{t-1}|Z_q^t,U_q^t)) \right]\,.
$$
In our case, this loss reduces to
$$
\mathcal{L}(\theta, \varphi) = \mathbb{E}_{q_\varphi} \left[ \log p_\theta(U_q^{0},Z_q^{0}|U_q^{0},Z_q^{0}) - \sum_{t=1}^{T}\mathrm{KL}(q_\varphi(U_q^{t-1}|U_q^{t},U_q^{0})\|p_\theta(U_q^{t-1}|U_q^t)) \right]\,.
$$
Bridge sampling quantizers then proceed as follows.
\begin{enumerate}
\item Define the noising transition density $q_\varphi(U_q^{t}|U_q^{t-1})$ as the transition density of a SDE.
\item Compute the bridge sampler $q_\varphi(U_q^{t-1}|U_q^{t},U_q^{0})$ or approximate it using Monte Carlo simulation and/or discretization schemes (see next session).
\item Define $p_\theta(U_q^{t-1}|U_q^t)$ as $q_\varphi(U_q^{t-1}|U_q^{t},U_q^{0})_{|U_q^{0} = g_\theta(U_q^t)}$ where $g_\theta$ is XXX.
\end{enumerate}
