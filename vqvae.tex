...
$$
\mathbb{P}_\theta(z_q= j) = \mu^{z_\rme}_{\theta}(j) \eqsp,
$$
where, for all $z\in\rset^m$, $\{\mu^z_{\theta}(j)\}_{1\leqslant j\leqslant K}$ is a probability vector on $\{1,\ldots, K\}$.
...
$$
where the $\varepsilon$ is a standard Gaussian random variables in $\rset^m$ and for all $z\in\rset^m$, $(\mu^{z},\sigma^{z}_\theta)$ are outputs of some neural network architecture to be defined  (proper neural network-based encoder  to be defined when the model is fixed).

\subsection{Loss function}
The (complete) loglikelihood of the model (conditionally on $X_I$ which is dropped from the notations) is given by
$$
\log p_{\theta}(z_q,Y) =  \log \mu^{z_\rme}_{\theta}(z_q) +  \log \varphi_{\mu^{z_q}_\theta,\sigma^{z_q}_\theta}(Y) \eqsp,
$$
...
$$
\mathbb{P}_\theta(z_q = \rme^j|Y) = \frac{\mu^{z_\rme}_{\theta}(j)\varphi_{\mu^{j}_\theta,\sigma^{j}_\theta}(Y)}{\sum_{k=1}^{K}\mu^{z_\rme}_{\theta}(k)\varphi_{\mu^{k}_\theta,\sigma^{k}_\theta}(Y)} = \omega^j_{z}\eqsp.
$$
Then, consider the loss function (Fisher or EM based update),
$$
\mathcal{L}(\theta) = \mathbb{E}\left[\log p_{\theta}(z_q,Y)  \middle | Y\right] = \sum_{j=1}^K  \omega^j_{z}\log p_{\theta}(\rme^j,Y)=  \sum_{j=1}^K  \omega^j_{z}\left\{\log \mu^{z_\rme}_{\theta}(\rme^j) +  \log \varphi_{\mu^{\rme^j}_\theta,\sigma^{\rme^j}_\theta}(Y)\right\}\eqsp.
$$
...
$$
\mathbb{P}_\theta(z^k_q = \rme^j,z^k_q = \rme^\ell|Y^{1:n}) =  \omega^{j,\ell}_{z|k,k+1}\eqsp.
$$
Then, consider the loss function (Fisher or EM based update),
\begin{align*}
\mathcal{L}(\theta) &= \mathbb{E}\left[\log p_{\theta}(z^{1:n}_q,Y^{1:n})  \middle | Y^{1:n}\right]\eqsp,\\
& =  \sum_{j=1}^K\omega^{j}_{z|1}\log \mu^{z_\rme^1}_{\theta}(\rme^j) + \sum_{k=0}^{n-1}\sum_{j,\ell=1}^K   \omega^{j,\ell}_{z|k,k+1}\left\{ \log  Q^{z_\rme^{k+1}}_{\theta}(\rme^j,\rme^\ell) +  \log \varphi_{\mu^{\rme^\ell}_\theta (Y^{k}),\sigma^{\rme^\ell}_\theta(Y^{k})}(Y_{k+1})  \right\}\eqsp.
\end{align*}
